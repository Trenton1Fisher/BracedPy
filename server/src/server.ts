      ...
      if (err) {
        ...
        if (typeof err.code === 'string' && err.code === 'ETIMEDOUT') {
          res.status(500).json({
            success: false,
            message:
              'Code execution took too long. Please double-check your loop conditions.',
          })
        } else {
          res.status(500).json({
            success: false,
        ...
        if (err) {
          ...
          if (err.signal === 'SIGTERM') {
            res.status(500).json({
              success: false,
              error:
                'Code execution took too long. Please double-check your loop conditions if any or double check syntax.',
            })
          } else {
            res.status(500).json({
              success: false,
